...
import {
  ...
  Gauge, HardDrive, LayoutDashboard, LoaderCircle, MapPin, MoreHorizontal, Pause,
  Play, Plus, Printer, RefreshCw, Search, Settings, SlidersHorizontal, Trash2,
  Upload, Wifi, WifiOff, X, Zap,
} from 'lucide-react'
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <div className="sidebar-version">NOVA FLEET · v0.1.0</div>
      </aside>
      ...
      <main className="main">
        <header className="topbar">
          ...
          <div className="top-actions">
            <button className="icon-button" title="Yenile" onClick={() => void refresh()} disabled={refreshing}><RefreshCw className={refreshing ? 'spin' : ''} size={18} /></button>
        ...
        {loading ? <div className="loading"><LoaderCircle className="spin" /><span>Filo taranıyor…</span></div> : (
          <div className="content">
            ...
            {view === 'printers' && <Printers snapshots={filtered} selected={selected} search={search} setSearch={setSearch} setSelectedId={setSelectedId} edit={(config) => setModal(config)} remove={removePrinter} uploadFile={(id) => void runAction(() => api.chooseAndUpload(id))} printFile={(id, name) => void runAction(() => api.printFile(id, name))} deleteFile={(id, name) => { if (window.confirm(`“${name}” silinsin mi?`)) void runAction(() => api.deleteFile(id, name)) }} control={(id, jobId, action) => void runAction(() => api.controlJob(id, jobId, action))} />}
            {view === 'files' && <FilesView snapshots={snapshots} selectedId={selectedId} setSelectedId={setSelectedId} uploadFile={(id) => void runAction(() => api.chooseAndUpload(id))} printFile={(id, name) => void runAction(() => api.printFile(id, name))} deleteFile={(id, name) => { if (window.confirm(`“${name}” silinsin mi?`)) void runAction(() => api.deleteFile(id, name)) }} />}
...
function Overview({ snapshots, online, printing, queuedFiles, averageProgress, search, setSearch, setSelected, uploadFile }: { snapshots: PrinterSnapshot[]; online: number; printing: number; queuedFiles: number; averageProgress: number; search: string; setSearch: (v: string) => void; setSelected: (id: string) => void; uploadFile: (id: string) => void }) {
  return <>
    ...
    <section className="bottom-grid"><div className="panel activity-panel"><div className="panel-title"><div><p className="section-kicker">SON HAREKETLER</p><h3>Akış</h3></div><button className="text-button">Tümünü gör</button></div><ActivityRow icon={<Play />} title="gearbox_v12.cws başlatıldı" detail="Reçine Lab 01" time="52 dk" /><ActivityRow icon={<Upload />} title="enclosure-final.cws yüklendi" detail="Reçine Lab 02" time="1 sa" /><ActivityRow icon={<Check />} title="bracket-set.cws tamamlandı" detail="Reçine Lab 02" time="3 sa" /></div><div className="panel health-panel"><p className="section-kicker">FİLO SAĞLIĞI</p><h3>Sistem özeti</h3><div className="health-ring"><div><strong>{Math.round((online / Math.max(1, snapshots.length)) * 100)}</strong><span>%</span></div></div><p>{online === snapshots.length ? 'Tüm sistemler normal çalışıyor.' : `${snapshots.length - online} yazıcı müdahale bekliyor.`}</p><div className="health-legend"><span><i className="ok" />Erişilebilir <b>{online}</b></span><span><i className="off" />Çevrimdışı <b>{snapshots.length - online}</b></span></div></div></section>
  </>
}
...
function PrinterCard({ snapshot, open, upload }: { snapshot: PrinterSnapshot; open: () => void; upload: () => void }) {
  ...
  return <article className={`printer-card ${snapshot.state}`} onClick={open}>
    <div className="card-top"><div className="printer-art"><Printer /></div><div className="card-menu"><span className={`status ${snapshot.state}`}><i />{statusLabel[snapshot.state]}</span><MoreHorizontal size={19} /></div></div>
...
function ActivityRow({ icon, title, detail, time }: { icon: React.ReactNode; title: string; detail: string; time: string }) { return <div className="activity-row"><span className="activity-icon">{icon}</span><div><strong>{title}</strong><small>{detail}</small></div><time>{time}</time></div> }

function Printers({ snapshots, selected, search, setSearch, setSelectedId, edit, remove, uploadFile, printFile, deleteFile, control }: { snapshots: PrinterSnapshot[]; selected?: PrinterSnapshot; search: string; setSearch: (v: string) => void; setSelectedId: (id: string) => void; edit: (c: PrinterConfig) => void; remove: (c: PrinterConfig) => void; uploadFile: (id: string) => void; printFile: (id: string, name: string) => void; deleteFile: (id: string, name: string) => void; control: (id: string, jobId: string, action: 'toggle' | 'stop') => void }) {
  return <div className="split-view"><section className="fleet-list"><div className="list-tools"><div className="search"><Search size={17} /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Filoda ara" /></div><button className="icon-button"><SlidersHorizontal size={17} /></button></div>{snapshots.map((snapshot) => <button key={snapshot.config.id} className={`fleet-row ${selected?.config.id === snapshot.config.id ? 'selected' : ''}`} onClick={() => setSelectedId(snapshot.config.id)}><span className={`device-icon ${snapshot.state}`}><Printer /></span><span className="fleet-name"><strong>{snapshot.config.name}</strong><small>{snapshot.config.model} · {snapshot.config.host}</small></span><span className={`status ${snapshot.state}`}><i />{statusLabel[snapshot.state]}</span><ChevronDown className="row-arrow" size={16} /></button>)}</section>{selected ? <PrinterDetail snapshot={selected} edit={edit} remove={remove} uploadFile={uploadFile} printFile={printFile} deleteFile={deleteFile} control={control} /> : <div className="panel detail-placeholder"><Printer /><p>Detaylarını görmek için bir yazıcı seçin.</p></div>}</div>
}

function PrinterDetail({ snapshot, edit, remove, uploadFile, printFile, deleteFile, control }: { snapshot: PrinterSnapshot; edit: (c: PrinterConfig) => void; remove: (c: PrinterConfig) => void; uploadFile: (id: string) => void; printFile: (id: string, name: string) => void; deleteFile: (id: string, name: string) => void; control: (id: string, jobId: string, action: 'toggle' | 'stop') => void }) {
  const { config, activeJob } = snapshot
  return <section className="panel printer-detail"><div className="detail-header"><div className="detail-avatar"><Printer /></div><div><span className={`status ${snapshot.state}`}><i />{statusLabel[snapshot.state]}</span><h2>{config.name}</h2><p>{config.model} · {config.location}</p></div><div className="detail-actions"><button className="icon-button" onClick={() => edit(config)}><Settings size={17} /></button><button className="icon-button danger" onClick={() => remove(config)}><Trash2 size={17} /></button></div></div><div className="detail-stats"><div><span>Adres</span><strong>{config.host}:{config.port}</strong></div><div><span>Yanıt</span><strong>{snapshot.latency ? `${snapshot.latency} ms` : '—'}</strong></div><div><span>Firmware</span><strong>{snapshot.firmware ?? 'Bilinmiyor'}</strong></div><div><span>Dosya alanı</span><strong>{formatBytes(snapshot.usedBytes)}</strong></div></div>{activeJob && <div className="active-job"><div className="active-job-head"><div><p className="section-kicker">AKTİF YAZDIRMA</p><h3>{activeJob.jobName}</h3></div><strong>%{activeJob.progress.toFixed(1)}</strong></div><div className="progress large"><i style={{ width: `${activeJob.progress}%` }} /></div><div className="job-stat-grid"><div><span>Katman</span><b>{activeJob.currentSlice} / {activeJob.totalSlices}</b></div><div><span>Katman kalınlığı</span><b>{activeJob.thickness} mm</b></div><div><span>Geçen süre</span><b>{formatDuration(activeJob.elapsedTime)}</b></div></div><div className="job-controls"><button className="secondary-button" onClick={() => control(config.id, activeJob.id, 'toggle')}>{snapshot.state === 'paused' ? <Play size={16} /> : <Pause size={16} />}{snapshot.state === 'paused' ? 'Sürdür' : 'Duraklat'}</button><button className="danger-button" onClick={() => control(config.id, activeJob.id, 'stop')}><CircleStop size={16} /> İşi durdur</button></div></div>}<div className="file-title"><div><p className="section-kicker">YAZICI DEPOSU</p><h3>Dosyalar <span>{snapshot.files.length}</span></h3></div><button className="secondary-button" onClick={() => uploadFile(config.id)} disabled={snapshot.state === 'offline'}><Upload size={16} /> Yükle</button></div><FileTable snapshot={snapshot} printFile={printFile} deleteFile={deleteFile} /></section>
}
...
function SettingsView({ configs, edit }: { configs: PrinterConfig[]; edit: (config: PrinterConfig) => void }) {
  return <div className="settings-grid"><section className="panel settings-card"><p className="section-kicker">BAĞLANTI POLİTİKASI</p><h2>Donanım dostu sorgulama</h2><p>Nova3D firmware’i yoğun HTTP trafiğinde yanıt vermeyi kesebilir. Nova Fleet istekleri yazıcı başına sıraya alır ve en az 5 saniyelik sorgulama aralığı uygular.</p><div className="setting-list"><span><Activity />Akıllı sorgulama <b>Açık</b></span><span><Wifi />Yerel ağ bağlantısı <b>HTTP :8081</b></span><span><HardDrive />Ayar deposu <b>Yerel</b></span></div></section><section className="panel settings-card"><p className="section-kicker">FİLO AYARLARI</p><h2>Yazıcı profilleri</h2>{configs.map((config) => <button className="settings-printer" key={config.id} onClick={() => edit(config)}><Printer /><span><strong>{config.name}</strong><small>{config.host}:{config.port} · {config.pollInterval} sn</small></span><Settings /></button>)}</section></div>
}

function PrinterModal({ value, close, save }: { value?: PrinterConfig; close: () => void; save: (input: SavePrinterInput) => Promise<void> }) {
  const initial = useMemo<SavePrinterInput>(() => value ?? { name: '', host: '', port: 8081, model: 'Nova3D Elfin', location: '', pollInterval: 10, enabled: true }, [value])
  const [form, setForm] = useState(initial)
  const [saving, setSaving] = useState(false)
  const field = (key: keyof SavePrinterInput, next: string | number | boolean) => setForm((current) => ({ ...current, [key]: next }))
  return <div className="modal-backdrop" onMouseDown={(e) => e.target === e.currentTarget && close()}><form className="modal" onSubmit={(e) => { e.preventDefault(); setSaving(true); void save(form).finally(() => setSaving(false)) }}><div className="modal-head"><div><p className="section-kicker">{value ? 'PROFİLİ DÜZENLE' : 'FİLOYA EKLE'}</p><h2>{value ? value.name : 'Yeni yazıcı'}</h2></div><button type="button" className="icon-button" onClick={close}><X /></button></div><div className="form-grid"><label className="full"><span>Görünen ad</span><input required value={form.name} onChange={(e) => field('name', e.target.value)} placeholder="Örn. Reçine Lab 01" /></label><label><span>IP adresi / sunucu</span><input required value={form.host} onChange={(e) => field('host', e.target.value)} placeholder="192.168.1.84" /></label><label><span>Port</span><input type="number" min="1" max="65535" value={form.port} onChange={(e) => field('port', Number(e.target.value))} /></label><label><span>Model</span><select value={form.model} onChange={(e) => field('model', e.target.value)}><option>Nova3D Elfin</option><option>Nova3D Bene4</option><option>Nova3D Whale3</option><option>Diğer Nova3D</option></select></label><label><span>Konum</span><input value={form.location} onChange={(e) => field('location', e.target.value)} placeholder="Örn. Prototip Atölyesi" /></label><label className="full"><span>Sorgulama aralığı</span><div className="range-row"><input type="range" min="5" max="60" value={form.pollInterval} onChange={(e) => field('pollInterval', Number(e.target.value))} /><b>{form.pollInterval} sn</b></div><small>Eski firmware sürümlerinde 10 saniye veya üzeri önerilir.</small></label></div><div className="modal-footer"><button type="button" className="secondary-button" onClick={close}>Vazgeç</button><button className="primary-button" disabled={saving}>{saving && <LoaderCircle className="spin" />} {value ? 'Değişiklikleri kaydet' : 'Yazıcıyı ekle'}</button></div></form></div>
}
