  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <div className="sidebar-version">NOVA FLEET · v0.3.0</div>
      </aside>
...
function JobsView({ snapshots, control }: { snapshots: PrinterSnapshot[]; control: (id: string, jobId: string, action: 'toggle' | 'stop') => void }) {
  ...
  const history = snapshots.flatMap((snapshot) => (snapshot.recentJobs ?? []).map((job) => ({ job, printer: snapshot.config }))).sort((a, b) => (b.job.endPrintTime ?? b.job.beginPrintTime ?? 0) - (a.job.endPrintTime ?? a.job.beginPrintTime ?? 0)).slice(0, 50)
  return <><div className="jobs-hero"><div><p className="section-kicker">CANLI KUYRUK</p><h2>{jobs.length ? `${jobs.length} aktif yazdırma` : 'Filo şu anda boşta'}</h2><p>Tüm makinelerdeki işleri tek ekrandan izleyin ve yönetin.</p></div><Activity size={58} /></div><div className="job-list">{jobs.map((item) => { const job = item.activeJob!; return <div className="panel job-row-card" key={job.id}><div className="job-device"><span className={`device-icon ${item.state}`}><Printer /></span><div><strong>{item.config.name}</strong><small>{job.jobName}</small></div></div><div className="job-progress-inline"><div><span>Katman {job.currentSlice} / {job.totalSlices}</span><b>%{job.progress.toFixed(1)}</b></div><div className="progress"><i style={{ width: `${job.progress}%` }} /></div></div><div className="job-time"><Clock3 /><span>Kalan<strong>{formatDuration((job.totalSlices - job.currentSlice) * job.averageSliceTime)}</strong></span></div><div className="file-actions"><button onClick={() => control(item.config.id, job.id, 'toggle')}>{item.state === 'paused' ? <Play /> : <Pause />}</button><button onClick={() => control(item.config.id, job.id, 'stop')}><CircleStop /></button></div></div>})}{!jobs.length && <Empty text="Aktif yazdırma işi bulunmuyor." />}</div><section className="history-section"><div className="section-heading"><div><p className="section-kicker">YAZDIRMA GEÇMİŞİ</p><h2>Son işler</h2></div><span className="history-count">{history.length} kayıt</span></div><div className="history-grid">{history.map(({ job, printer }) => <article className="panel history-card" key={`${printer.id}-${job.id}`}><div className="history-head"><div><strong>{job.jobName}</strong><small>{printer.name}</small></div><span className="status online"><i />{job.status || 'Tamamlandı'}</span></div><div className="history-stats"><span>Katman<b>{job.currentSlice} / {job.totalSlices}</b></span><span>Süre<b>{formatDuration(job.elapsedTime)}</b></span><span>Kalınlık<b>{job.thickness} mm</b></span><span>Pozlama<b>{job.totalExposureTime ?? job.layerTime ? `${job.totalExposureTime ?? (job.layerTime ?? 0) / 1000} sn` : '—'}</b></span><span>Z kaldırma<b>{job.zliftDistance ? `${job.zliftDistance} mm` : '—'}</b></span><span>Hız<b>{job.zliftSpeed ? `${job.zliftSpeed} mm/dk` : '—'}</b></span></div>{job.endPrintTime ? <time>{new Date(job.endPrintTime).toLocaleString('tr-TR')}</time> : null}</article>)}{!history.length && <Empty text="Henüz tamamlanmış iş bulunmuyor." />}</div></section></>
}
...
function PrinterModal({ value, close, save }: { value?: PrinterConfig; close: () => void; save: (input: SavePrinterInput) => Promise<void> }) {
  const initial = useMemo<SavePrinterInput>(() => value ?? { name: '', host: '', port: 8081, model: 'Otomatik algılanıyor', location: '', pollInterval: 10, enabled: true }, [value])
